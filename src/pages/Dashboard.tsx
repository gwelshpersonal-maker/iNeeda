...
const ServiceCard = ({
  ...
  <button
    ...
    ${
      disabled
        ? "opacity-50 cursor-not-allowed bg-slate-50 border border-slate-100"
        : isEmergency
  ...
  >
    <div
      className={`p-5 rounded-2xl transition-transform duration-300 group-hover:scale-110 ${isEmergency ? "bg-red-100 text-red-600" : "bg-slate-50 group-hover:bg-white shadow-sm"}`}
    >
      <Icon className={`w-8 h-8 ${colorClass}`} />
    </div>
    <h3
      className={`text-lg font-bold tracking-tight ${isEmergency ? "text-red-700" : "text-navy-900"}`}
    >
      {title}
    </h3>
    {disabled && (
      <span className="absolute bottom-4 text-[10px] text-slate-400 font-bold uppercase bg-slate-100 px-2 py-1 rounded">
        No Providers
      </span>
    )}
  </button>
...
export const DynamicCategoryIcon: React.FC<{
  name: string;
  className?: string;
}> = ({ name, className }) => {
...
export const Dashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<"services" | "calendar">(
    "services",
  );
  ...
  useEffect(() => {
    if (isAdmin && shifts) {
      shifts.forEach((shift) => {
        ...
        const feePercent =
          shift.appliedPlatformFee !== undefined
        ...
        if (shift.insuranceOptIn) {
          if (shift.appliedInsuranceFee !== undefined)
            ...
            const rule = platformConfig[shift.category]?.insuranceRule || {
              type: "FLAT",
              value: 2.0,
            };
            insuranceFee =
              rule.type === "PERCENTAGE"
                ? gross * (rule.value / 100)
                : rule.value;
          }
        }
        ...
        if (net < 0 || gross < 0) {
          console.log(
            `Deleting erroneous negative price/net shift: ${shift.id}, gross: ${gross}, net: ${net}`,
          );
  ...
  const isVerifiedInsured = useMemo(() => {
    ...
    return (
      currentUser.isCoiVerified || currentUser.insuranceType === "DAILY_SHIELD"
    );
  ...
  useEffect(() => {
    const rebookProId = searchParams.get("rebook");
    const rebookCategory = searchParams.get("category");
    const templateId = searchParams.get("templateId");
    const isDraft = searchParams.get("draft") === "true";
    const draftDesc = searchParams.get("desc");

    let opened = false;

    if (isDraft && rebookCategory && draftDesc) {
      setSelectedCategory(rebookCategory as ServiceCategory);
      setRequestDesc(draftDesc);
      opened = true;
      searchParams.delete("draft");
      searchParams.delete("category");
      searchParams.delete("desc");
      setSearchParams(searchParams);
    } else if (rebookProId && rebookCategory) {
      setSelectedCategory(rebookCategory as ServiceCategory);
      setTargetCrewIds([rebookProId]);
    ...
    if (templateId) {
      const oldShift = shifts.find((s) => s.id === templateId);
      if (oldShift) {
        setSelectedCategory(oldShift.category);
        setRequestDesc(oldShift.description);
        setEstimatedPrice(oldShift.price?.toString() || "");
        if (oldShift.siteId) {
          setSelectedSiteId(oldShift.siteId);
        }
        opened = true;
      }
  ...
  }, [searchParams, setSearchParams, shifts]);
  ...
  const [marketEstimate, setMarketEstimate] =
    useState<MarketPriceEstimate | null>(null);
  const [isEstimating, setIsEstimating] = useState(false);

  const handleSaveInlineSite = async () => {
    if (!newLocationName.trim() || !newLocationAddress.trim() || !currentUser)
      return;
  ...
  const handleRefineDescription = async () => {
    ...
    try {
      ...
      alert(
        `AI Refinement failed: ${error.message || "Check your AI configuration."}`,
      );
    } finally {
      setIsRefining(false);
    }
  ...
  const handleGetMarketEstimate = async () => {
    ...
    try {
      ...
      alert(
        `Market Estimate failed: ${error.message || "Check your AI configuration."}`,
      );
    } finally {
  ...
  const [showBackgroundCheckPayment, setShowBackgroundCheckPayment] =
    useState(false);
  ...
  const [emergencyConfirmGig, setEmergencyConfirmGig] = useState<Shift | null>(
    null,
  );
  ...
  const [estimatedInsuranceLabel, setEstimatedInsuranceLabel] =
    useState<string>("");
  const [willDeductInsurance, setWillDeductInsurance] = useState(false);

  // Skill Application State
  const [skillApplyCategory, setSkillApplyCategory] =
    useState<ServiceCategory | null>(null);
  ...
  const alert = (message: string, type: "success" | "error" | "info" = "error") => {
    let alertType: "success" | "error" | "info" = type;
    const lower = message.toLowerCase();
    if (lower.includes("success") || lower.includes("approved")) {
      alertType = "success";
    } else if (lower.includes("fail") || lower.includes("error") || lower.includes("cannot") || lower.includes("invalid") || lower.includes("missing") || lower.includes("pending")) {
      alertType = "error";
    } else {
      alertType = "info";
    }
  ...
  const handlePhoneClick = (e: React.MouseEvent<HTMLAnchorElement>, phone: string) => {
    ...
    navigator.clipboard.writeText(phone).then(() => {
      setToast({ message: `Phone number copied: ${phone}`, type: "success" });
    }).catch((err) => {
  ...
  const isSubscribed = useMemo(() => {
    ...
    return (
      ["active", "trialing"].includes(currentUser?.subscriptionStatus || "") ||
      currentUser?.isFoundersClub === true
    );
  }, [isProvider, currentUser]);
  ...
  const handleGenerateQuote = async () => {
    ...
    try {
      const generated = await generateProfessionalQuote(
        quoteGig.description,
  ...
  const [referralType, setReferralType] = useState<"CLIENT" | "PROVIDER">(
    "CLIENT",
  );
  ...
  const [facebookShareData, setFacebookShareData] = useState<{
    gig: Shift;
    rating: number;
    feedback: string;
    providerName: string;
  ...
  const handleVerifyCompletion = async () => {
    ...
    try {
      ...
      if (verificationRating === 5) {
        const providerName =
          users.find((u) => u.id === verificationGig.userId)?.name || "a Pro";
        setFacebookShareData({
          gig: verificationGig,
          rating: verificationRating,
          feedback: verificationFeedback,
  ...
  const providerQualifications = useMemo(() => {
    ...
    const generalLaborCount = shifts.filter(
      (s) =>
        s.userId === currentUser.id &&
        s.category === "GENERAL_LABOR" &&
        (s.status === ShiftStatus.COMPLETED ||
          s.status === ShiftStatus.VERIFIED),
    ).length;

    const { numericRating: rating } = getProviderStats(
      currentUser.id,
      shifts,
      currentUser,
    );
  ...
  const totalEarnings = useMemo(() => {
    ...
    return shifts
      ...
      .reduce((total, s) => {
        ...
        const feePercent =
          s.appliedPlatformFee !== undefined
        ...
        if (s.insuranceOptIn) {
          if (s.appliedInsuranceFee !== undefined) {
            ...
            const rule = platformConfig[s.category]?.insuranceRule || {
              type: "FLAT",
              value: 2.0,
            };
            insuranceFee =
  ...
  const startRequest = (category: ServiceCategory) => {
    ...
    setSelectionMethod(
      riskLevel === RISK_LEVELS.HIGH ? "BIDDING" : "QUICK_BID",
    );
  ...
  const handleEditClick = (e: React.MouseEvent, gig: Shift) => {
    ...
    setRequestedDate(
      gig.type === "SCHEDULED" && gig.start
        ? new Date(gig.start).toISOString().split("T")[0]
        : "",
    );
    ...
    const riskLevel =
      CATEGORY_RISK_MAPPING[gig.category]?.risk || RISK_LEVELS.LOW;
    setSelectionMethod(
      gig.selectionMethod ||
        (riskLevel === RISK_LEVELS.HIGH ? "BIDDING" : "QUICK_BID"),
    );
  ...
  const submitRequest = async () => {
    if (!selectedCategory || !selectedSiteId || !requestDesc || !currentUser)
      return;
    ...
    if (
      selectedSiteId.startsWith("home_") &&
    ...
    const riskLevel =
      CATEGORY_RISK_MAPPING[selectedCategory]?.risk || RISK_LEVELS.LOW;
    ...
    const PROVIDER_EMERGENCY_SHARE = 0.6;
    ...
    if (editingGig) {
      ...
      const updatedGig: Shift = {
        ...
        start: requestedDate ? new Date(requestedDate) : new Date(),
          ...
          : requestedDate
            ? addHours(new Date(requestedDate), 2)
            : addHours(new Date(), 2),
        price: finalPrice,
        ...
        recurringFrequency: isRecurringJob ? recurringFrequency : undefined,
    ...
    const groupId = workerCount > 1 ? `group_${Date.now()}` : undefined;

    for (let i = 0; i < workerCount; i++) {
      const shiftId = `gig_${Date.now()}_${i}`;
      const newGig: Shift = {
        id: shiftId,
        ...
        start: requestedDate ? new Date(requestedDate) : new Date(), // Immediate/ASAP or Scheduled
          ...
          : requestedDate
            ? addHours(new Date(requestedDate), 2)
            : addHours(new Date(), 2), // Default duration
        description: requestDesc,
        category: selectedCategory,
        ...
        groupId: groupId, // Links these jobs together for Capacity Management
      };
    ...
    if (isUrgent) {
      ...
      const eligibleProviders = users.filter((u) => {
        if (
          ...
          !u.skills?.includes(selectedCategory)
        ) {
          return false;
        }
      ...
      eligibleProviders.forEach((p) => {
        ...
        console.log(
          `%c[SMS SIMULATION] To: ${p.phone} | Msg: URGENT ${selectedCategory}: ${requestDesc}`,
      ...
      alert(
        "Your request has been broadcasted" +
          (targetCrewIds.length > 0
            ? " and emails have been sent to your targeted crew!"
            : "!"),
      );
    }
  };
  ...
  const handleBoost = async (shift: Shift) => {
    ...
    const BOOST_PERCENT = 0.1; // 10% increase
    const minIncrease = 5;
  ...
  const executeUpgradeToEmergency = () => {
    ...
    const currentPrice =
      typeof emergencyConfirmGig.price === "string"
        ? parseFloat(emergencyConfirmGig.price)
        : emergencyConfirmGig.price || 0;
    ...
    // Simulate broadcast
    const eligibleProviders = users.filter(
      (u) =>
        u.role === Role.PROVIDER &&
        u.isActive &&
        u.urgentAlertsEnabled &&
  ...
  const priceSuggestion = useMemo(() => {
    ...
    return {
      ...
      count: prices.length,
      isSpecific,
      examples: matchedShifts.slice(0, 5),
    };
  ...
  const handleClaimClick = (
    ...
    if (!currentUser.isActive) {
      alert(
        "Your account is pending admin activation. You will be able to claim or bid on jobs once your background check is verified.",
      );
      return;
    }
    ...
    if (confirmingGigId === gig.id) {
      ...
      const shouldDeduct =
        forceDailyShield ||
        currentUser.insuranceType === "DAILY_SHIELD" ||
        (currentUser.insuranceType === "OWN_COI" && !currentUser.isCoiVerified);
      setWillDeductInsurance(shouldDeduct);

      if (shouldDeduct) {
        const rule = platformConfig[gig.category]?.insuranceRule || {
          type: "FLAT",
          value: 2.0,
        };
  ...
  const submitSkillApplication = () => {
    ...
    alert(
      `Application sent! An admin will review your qualifications for ${skillApplyCategory}.`,
    );
  };
  ...
  const handleOpenCounterModal = (e: React.MouseEvent, gig: Shift) => {
    ...
    setCounterMessage(
      "I'm interested, but I'd like to propose a different price.",
    );
  ...
  const handleCounterSubmit = async () => {
    console.log("handleCounterSubmit triggered", {
      counterGig,
      currentUser,
      counterAmount,
    });
      ...
      if (counterDate && counterTime) {
        ...
        if (!isNaN(d.getTime())) {
          ...
          if (counterEndDate && counterEndTime) {
            ...
            suggestedEnd = !isNaN(endD.getTime())
              ? endD
              : addHours(suggestedStart, 2);
          } else {
  ...
  const handleOpenQuoteModal = (e: React.MouseEvent, gig: Shift) => {
    ...
    const existingQuote = gig.quotes?.find(
      (q) => q.providerId === currentUser?.id,
    );
    if (existingQuote) {
  ...
  const handleQuoteSubmit = async () => {
    console.log("handleQuoteSubmit triggered", {
      quoteGig,
      currentUser,
      quoteAmount,
    });
    ...
    try {
      ...
      const hasOwnInsurance =
        provider?.insuranceType === "OWN_COI" && provider?.isCoiVerified;
      const isEmergency = quoteGig.type === "URGENT";
      const rule = platformConfig?.[quoteGig.category];
      const siteAddress =
        sites.find((s) => s.id === quoteGig.siteId)?.address || "";
      const breakdown = calculateJobSplit(
        amount,
        quoteGig.category,
        hasOwnInsurance,
        isEmergency,
        rule?.rateCategory || "STANDARD",
        rule?.insuranceRule?.value || null,
        siteAddress,
      );

      const existingQuote = quoteGig.quotes?.find(
        (q) => q.providerId === currentUser.id,
      );
    ...
    setReviewOffersGig((prev) => {
      ...
      return {
        ...
        quotes: prev.quotes?.map((q) =>
          q.id === quote.id ? { ...q, status: "DECLINED" } : q,
        ),
      };
    });
  ...
  const handleAcceptQuote = async (
    ...
    const shouldDeduct = winningProvider
      ? winningProvider.insuranceType === "DAILY_SHIELD" ||
        (winningProvider.insuranceType === "OWN_COI" &&
          !winningProvider.isCoiVerified)
      : false;

    let insuranceFee = 0;
    if (shouldDeduct) {
      const rule = config?.insuranceRule || { type: "FLAT", value: 2.0 };
      insuranceFee =
        rule.type === "PERCENTAGE"
          ? quote.amount * (rule.value / 100)
          : rule.value;
    }
    ...
    const siteAddress =
      sites.find((s) => s.id === reviewOffersGig.siteId)?.address || "";
    const amountWithTaxBreakdown = calculateJobSplit(
      quote.amount,
      reviewOffersGig.category,
      !shouldDeduct,
      false,
      config?.rateCategory || "STANDARD",
      insuranceFee,
      siteAddress,
    );
  ...
  const handlePaymentSuccess = async (paymentIntentId: string) => {
    ...
    try {
      ...
      const newStart = pendingPayment.finalDate
        ? new Date(pendingPayment.finalDate)
        : gig.start;
      let newEnd = gig.end;
      ...
      const updatedQuotes = gig.quotes?.map((q) => ({
        ...q,
        status:
          pendingPayment.quoteId && q.id === pendingPayment.quoteId
            ? ("ACCEPTED" as const)
            : ("DECLINED" as const),
      }));
      ...
      const updatedOffers = gig.counterOffers?.map((o) => ({
        ...o,
        status:
          pendingPayment.offerId && o.id === pendingPayment.offerId
            ? ("ACCEPTED" as const)
            : ("REJECTED" as const),
      }));
      ...
      alert(
        `Payment successful! Funds for "${gig.description}" are now secured in escrow.`,
      );
    } catch (error: any) {
  ...
  const myNotifications = notifications.filter(
    (n) => n.targetUserId === currentUser?.id,
  );
  ...
  const availableGigs = shifts.filter((s) => {
    ...
    if (s.groupId && currentUser) {
      const alreadyClaimed = shifts.some(
        (existingShift) =>
          existingShift.groupId === s.groupId &&
          existingShift.userId === currentUser.id &&
          existingShift.status !== ShiftStatus.CANCELLED,
      );
    ...
    const isMyTargeted =
      s.targetedProviders &&
      s.targetedProviders.includes(currentUser?.id || "");
  ...
  const myActiveGigs = shifts.filter(
    (s) =>
  ...
  const pendingProviders = users.filter(
    (u) => u.role === Role.PROVIDER && u.verificationStatus === "PENDING",
  ).length;
  const activeGigsCount = shifts.filter(
    (s) =>
      s.status === ShiftStatus.ACCEPTED || s.status === ShiftStatus.IN_PROGRESS,
  ).length;
  const openRequestsCount = shifts.filter(
    (s) => s.status === ShiftStatus.OPEN_REQUEST,
  ).length;

  const hasAuthorizedSkills =
    isProvider && currentUser?.skills && currentUser.skills.length > 0;

  const unclaimedByCategory = useMemo(() => {
    const counts: Record<string, number> = {};
    shifts
      .filter((s) => s.status === ShiftStatus.OPEN_REQUEST)
      .forEach((s) => {
  ...
  const todoJobs = useMemo(() => {
    ...
    return shifts.filter((s) => {
      ...
      if (s.status === ShiftStatus.ACCEPTED && s.escrowStatus === "PENDING")
        return true;

      // 3. Jobs with pending counter offers
      const hasPendingCounter = s.counterOffers?.some(
        (o) => o.status === "PENDING",
      );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {todoJobs.map((job) => {
            ...
            const isAwaitingFunding =
              job.status === ShiftStatus.ACCEPTED &&
              job.escrowStatus === "PENDING";
            const hasPendingCounter = job.counterOffers?.some(
              (o) => o.status === "PENDING",
            );
            const hasQuotes =
              job.selectionMethod === "BIDDING" &&
            ...
            return (
              <div
                ...
                onClick={() => {
                  if (isCompleted) {
                    ...
                    const siteAddress =
                      sites.find((s) => s.id === job.siteId)?.address || "";
                    const amountWithTaxBreakdown = calculateJobSplit(
                      job.price || 0,
                      job.category,
                      !job.insuranceOptIn,
                      false,
                      job.rateCategory || "STANDARD",
                      job.appliedInsuranceFee || 0,
                      siteAddress,
                    );
                    setPendingPayment({
                      amount: amountWithTaxBreakdown.clientTotalAmount!,
              ...
              >
                <div
                  className={`absolute top-0 left-0 w-1 h-full ${isCompleted ? "bg-emerald-500" : hasPendingCounter ? "bg-amber-500" : "bg-indigo-500"}`}
                ></div>
                <div className="flex justify-between items-start mb-2">
                  <span
                    className={`text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider border ${statusColor}`}
                  >
                    {statusLabel}
                  </span>
  ...
  const renderServiceGrid = () => {
    const displayCategories = serviceCategories.filter(
      (c) => c.isPublic && c.isActive,
    );
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        {isClientReferralEnabled && (
          <ServiceCard
            ...
            icon={Icons.Gift as any}
            colorClass="text-pink-500"
            onClick={() => openReferralModal("CLIENT")}
          />
        )}
        <button
        ...
        >
          ...
          <div className="text-center">
            <h3 className="text-base font-bold text-slate-500 group-hover:text-navy-900">
              Request Category
            </h3>
  ...
  const renderGigCard = (gig: Shift) => {
    ...
    const myPendingOffer = gig.counterOffers?.find(
      (o) => o.providerId === currentUser.id && o.status === "PENDING",
    );
    const missingSkill = !currentUser.skills?.includes(gig.category);
    const isLocked =
      gig.hasHighValueItems && !providerQualifications.canClaimHighValue;
    ...
    return (
      <div
        key={gig.id}
        className={`p-6 rounded-2xl shadow-soft hover:shadow-xl transition-all relative overflow-hidden mb-4 ${isEmergency ? "bg-red-50/50 border border-red-100" : isBoosted ? "bg-gradient-to-br from-white to-purple-50 border border-purple-100" : "bg-white border border-transparent"} ${(isRiskBlocked && !isConfirming) || missingSkill ? "opacity-90" : ""}`}
      >
        <div className="absolute top-0 right-0 flex flex-col items-end gap-1 z-10">
          {isBoosted && (
            <div className="bg-purple-600 text-white text-[10px] font-bold px-3 py-1 rounded-bl-xl shadow-sm flex items-center">
              <Rocket className="w-3 h-3 mr-1" /> Price Boosted!
            </div>
          )}
          {gig.hasHighValueItems && (
            <div className="bg-navy-800 text-gold-400 text-[10px] font-bold px-3 py-1 rounded-bl-xl shadow-sm flex items-center border-l border-b border-gold-500">
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1 pr-4">
            <span
              className={`text-[10px] font-bold px-3 py-1 rounded-full mb-3 inline-block tracking-wide uppercase ${isEmergency ? "bg-red-600 text-white animate-pulse" : "bg-navy-50 text-navy-700"}`}
            >
              {gig.category}
            </span>
            ...
            <div className="relative">
              <h3
                className={`font-extrabold text-xl text-navy-900 leading-tight ${expandedGigId === gig.id ? "" : "line-clamp-4"}`}
              >
                {gig.description}
              </h3>
              {gig.description.length > 150 && (
                ...
                >
                  {expandedGigId === gig.id ? "Show Less" : "Read More"}{" "}
                  <ChevronRight
                    className={`w-3 h-3 ml-1 transition-transform ${expandedGigId === gig.id ? "-rotate-90" : "rotate-90"}`}
                  />
                </button>
              )}
            </div>
              ...
              <div className="flex gap-3 mt-2 text-xs font-bold text-slate-600">
                {gig.truckNeeded && (
                  <span className="bg-amber-100 text-amber-800 px-2 py-1 rounded flex items-center">
                    <Truck className="w-3 h-3 mr-1" /> Truck Required
                  </span>
                )}
                {gig.distance && (
                  <span className="bg-slate-100 px-2 py-1 rounded flex items-center">
                    <Milestone className="w-3 h-3 mr-1" /> {gig.distance}
                  </span>
                )}
              </div>
            )}
          ...
          <div className="text-right pt-6">
            <div
              className={`text-xl font-bold tracking-tight text-slate-400 line-through decoration-slate-300 decoration-2`}
            >
              ${gig.price}
            </div>
        ...
        <div className="mt-4 bg-slate-50 rounded-xl p-3 border border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            {breakdown.insuranceFee > 0 && (
              <div className="text-center border-l border-slate-200 pl-4">
                <div className="text-[10px] text-blue-400 font-bold uppercase flex items-center justify-center">
                  Shield <ShieldCheck className="w-3 h-3 ml-1" />
                </div>
                <div className="text-xs font-bold text-blue-600">
                  -${breakdown.insuranceFee.toFixed(2)}
                </div>
              </div>
            )}
            <div className="text-center border-l border-slate-200 pl-4">
              <div
                className="text-[10px] text-slate-400 font-bold uppercase flex items-center justify-center cursor-help"
                title="Recommendation only. You receive this money but should save it for taxes."
              >
                Rec. Save <Wallet className="w-3 h-3 ml-1" />
              </div>
              <div className="text-xs font-bold text-slate-500 italic">
                ~${breakdown.taxHoldbackEstimate.toFixed(2)}
              </div>
            </div>
          </div>
          <div className="text-right pl-4 border-l border-slate-200">
            ...
            <div className="text-xl font-black text-emerald-600">
              ${breakdown.providerNet.toFixed(2)}
            </div>
          </div>
        </div>
        ...
        {missingSkill ? (
          <div className="mt-6 bg-slate-100 rounded-xl p-4 border border-slate-200 text-center">
            ...
            <p className="font-bold text-slate-700 text-sm">
              You are not currently authorized for this skill set.
            </p>
            <p className="text-xs text-slate-500 mt-1 mb-4">
              If you are interested in adding a skill set, apply here.
            </p>
            {currentUser.pendingSkills?.includes(gig.category) ? (
              <div className="w-full py-2 bg-amber-50 text-amber-700 border border-amber-200 rounded-lg text-xs font-bold shadow-sm cursor-default flex items-center justify-center">
                <Clock className="w-4 h-4 mr-1.5" /> Application Pending
              </div>
            ) : (
              <button
                onClick={() => handleOpenSkillApplication(gig.category)}
                className="w-full py-2 bg-white text-navy-600 border border-navy-200 rounded-lg text-xs font-bold hover:bg-navy-50 transition-colors shadow-sm flex items-center justify-center"
              >
                Apply for {gig.category} Authorization
              </button>
            )}
          </div>
          ...
          <div className="mt-6 bg-amber-50 rounded-xl p-4 border border-amber-200">
            <div className="flex items-start text-amber-800 text-sm font-bold mb-3">
              ...
              <div>
                <p>{riskStatus.reason}</p>
                <p className="text-xs font-normal mt-1 opacity-90">
                  This is a high-liability category. Proof of insurance is
                  required.
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              {riskStatus.actionRequired === "OPT_IN_SHIELD" && (
                <button
                  onClick={(e) => handleClaimClick(e, gig, true)}
                  className="w-full py-2 bg-white text-amber-700 border border-amber-200 rounded-lg text-xs font-bold hover:bg-amber-100 transition-colors shadow-sm"
                >
                  Opt-in to Daily Shield ($
                  {breakdown.insuranceFee > 0
                    ? breakdown.insuranceFee.toFixed(2)
                    : "2.00"}
                  )
              ...
              {riskStatus.actionRequired === "UPLOAD_COI" && (
                <button
                  onClick={() => navigate("/profile")}
                  className="w-full py-2 bg-white text-navy-600 border border-navy-200 rounded-lg text-xs font-bold hover:bg-navy-50 transition-colors shadow-sm"
                >
                  Upload COI
                </button>
              )}
          ...
          <div className="mt-6 bg-slate-100 rounded-xl p-3 border border-slate-200 flex items-center text-slate-500 text-xs font-bold">
            <Lock className="w-4 h-4 mr-2" />
            <span>Requires 4.5★ or 10+ General Labor jobs.</span>
          </div>
        ) : (
          <>
            {isConfirming && (
              <div className="mt-6 mb-2 bg-blue-50 border border-blue-100 p-3 rounded-xl flex items-center justify-between animate-in fade-in cursor-default">
                <div className="flex items-center">
                  <ShieldCheck
                    className={`w-5 h-5 mr-2 ${willDeductInsurance ? "text-blue-600" : "text-emerald-600"}`}
                  />
                  <div>
                    <p className="text-xs font-bold text-navy-900">
                      {willDeductInsurance
                        ? "Daily Shield Applied"
                        : "Insurance Verified"}
                    </p>
                    <p className="text-xs text-slate-600">
                      {willDeductInsurance
                        ? "Deduction for this job:"
                        : "Own coverage active. No deduction."}
                    </p>
                  </div>
                </div>
                {willDeductInsurance && (
                  <div className="bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded">
                    - ${estimatedInsuranceFee.toFixed(2)}{" "}
                    {estimatedInsuranceLabel}
                  </div>
                )}
              </div>
            )}
            <div className="flex gap-3 mt-4">
              {isBidding ? (
                !isSubscribed ? (
                  <button
                    onClick={() => navigate("/profile")}
                    className="w-full py-3 bg-slate-50 text-slate-500 font-bold rounded-xl border-2 border-dashed border-slate-200 hover:bg-gold-50 transition-all flex items-center justify-center"
                  >
                    <Lock className="w-5 h-5 mr-2" /> Subscribe to Bid
                  </button>
                ) : hasQuoted ? (
                  <button
                    onClick={(e) => handleOpenQuoteModal(e, gig)}
                    className="w-full py-3 bg-amber-500 text-white font-bold rounded-xl hover:bg-amber-600 shadow-md hover:shadow-lg transition-all flex items-center justify-center"
                  >
                  ...
                  <button
                    onClick={(e) => handleOpenQuoteModal(e, gig)}
                    className="w-full py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-md hover:shadow-lg transition-all flex items-center justify-center"
                  >
                ...
                <>
                  <button
                    ...
                    onClick={(e) =>
                      isSubscribed
                        ? handleClaimClick(e, gig)
                        : navigate("/profile")
                    }
                    disabled={isClaiming}
                    className={`flex-1 py-3 rounded-xl font-bold transition-all transform active:scale-[0.98] flex items-center justify-center shadow-md ${!isSubscribed ? "bg-slate-50 text-slate-500 border-2 border-dashed border-slate-200 hover:bg-gold-50 cursor-pointer" : isClaiming ? "bg-slate-200 text-slate-400 cursor-wait" : isConfirming ? "bg-emerald-500 text-white hover:bg-emerald-600 shadow-emerald-200" : isEmergency ? "bg-red-600 text-white hover:bg-red-700 shadow-red-200" : "bg-navy-900 text-white hover:bg-navy-800 hover:shadow-lg"}`}
                  >
                    {!isSubscribed ? (
                      <>
                        <Lock className="w-4 h-4 mr-2" /> Subscribe to Claim
                      </>
                    ) : isClaiming ? (
                      <>
                  ...
                  {isConfirming ? (
                    <button
                      ...
                      className="px-4 py-3 rounded-xl font-bold border-2 border-slate-200 bg-white text-slate-600 hover:bg-slate-50 hover:text-navy-950 transition-all flex items-center justify-center cursor-pointer"
                    >
                      Cancel
                    </button>
                  ) : (
                    <button
                      ...
                      onClick={(e) =>
                        isSubscribed
                          ? handleOpenCounterModal(e, gig)
                          : navigate("/profile")
                      }
                      disabled={!!myPendingOffer}
                      className={`px-4 py-3 rounded-xl font-bold border-2 transition-all flex items-center justify-center ${myPendingOffer ? "bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed" : "bg-white border-slate-200 text-slate-600 hover:border-gold-400 hover:text-navy-900 hover:bg-gold-50 cursor-pointer"}`}
                      title={
                        myPendingOffer
                          ? "Offer Pending"
                    ...
                    >
                      {myPendingOffer ? "Offer Sent" : "Counter Offer"}
                    </button>
                  )}
                </>
              )}
  ...
  const handleApproveCategoryRequest = async (request: CategoryRequest) => {
    ...
    const newCategoryId = request.categoryName
      .toUpperCase()
      .replace(/[^A-Z0-9]/g, "_");
    ...
    addServiceCategory({
      ...
      description:
        request.description || `New category requested by the community.`,
    ...
    alert(
      `"${request.categoryName}" approved! Routing you to the Catalog CMS to finalize the risk level and public visibility.`,
    );
    // Switch the admin to the Admin Dashboard (Catalog)
    navigate("/admin");
  };
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="w-0 flex-1 flex items-center">
              ...
              <p className="ml-3 font-medium text-amber-900 truncate">
                ...
                <span className="hidden md:inline">
                  Your Pro membership is currently inactive. Subscribe to unlock
                  bidding and job claims.
                </span>
              </p>
            </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium text-lg mt-1">
            {isClient
              ? "we've got a pro for that."
              : isAdmin
                ? "Platform Overview & Approvals"
                : "Pick up a job and get paid."}
          </p>
        </div>
        <div className="flex items-center gap-3 self-end md:self-auto">
          <div className="relative">
            <button
              ...
              className="p-3 bg-white text-navy-600 rounded-full shadow-soft hover:shadow-lg hover:scale-105 transition-all relative border border-slate-100"
            >
              <Bell className="w-6 h-6" />
              {unreadCount > 0 && (
                <span className="absolute top-0 right-0 w-3 h-3 bg-red-500 rounded-full border-2 border-white ring-2 ring-red-100"></span>
              )}
            </button>
            {isNotifOpen && (
              <div className="absolute right-0 mt-3 w-72 sm:w-80 md:w-96 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden animate-in slide-in-from-top-5">
                <div className="p-4 border-b border-slate-50 font-bold text-navy-900 flex justify-between items-center">
                ...
                <div className="max-h-[400px] overflow-y-auto">
                  {myNotifications.length === 0 ? (
                    ...
                    myNotifications.map((n) => (
                      <div
                        key={n.id}
                        className={`p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors text-sm ${n.type === "ALERT" ? "bg-red-50/50 text-red-900 font-medium" : ""} relative group`}
                      >
                        <div className="pr-6">
                          {n.type === "ALERT" && (
                            <Siren className="w-4 h-4 inline mr-2 text-red-500" />
                          )}
                          {n.message}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            <div className="flex justify-between items-center mb-6">
              ...
              {currentUser && (
                <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-soft hover:shadow-md transition-all cursor-pointer">
                  <label className="flex items-center cursor-pointer">
                    <div className="relative">
                      <input
                        type="checkbox"
                        className="sr-only"
                        checked={currentUser.urgentAlertsEnabled}
                        onChange={toggleUrgentAlerts}
                      />
                      <div
                        className={`block w-9 h-5 rounded-full transition-colors ${currentUser.urgentAlertsEnabled ? "bg-red-500" : "bg-slate-300"}`}
                      ></div>
                      <div
                        className={`dot absolute left-1 top-1 bg-white w-3 h-3 rounded-full transition-transform ${currentUser.urgentAlertsEnabled ? "transform translate-x-4" : ""}`}
                      ></div>
                    </div>
                    <span className="ml-3 text-xs font-bold text-navy-900 uppercase tracking-wide">
                      Emergency Texts
                    </span>
                  </label>
                </div>
                ...
                <div className="flex items-start">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-blue-800">
                      Verify to unlock all jobs
                    </h3>
                    <p className="text-xs text-blue-700 mt-1">
                      Some high-risk categories are locked until your insurance
                      is verified.
                    </p>
                    <button
                      onClick={() => navigate("/profile")}
                      className="mt-2 text-xs font-bold bg-white text-blue-600 px-3 py-1 rounded border border-blue-200 shadow-sm hover:bg-blue-50"
                    >
                      Update Insurance
                    </button>
                  </div>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6 bg-white p-4 rounded-xl shadow-sm border border-slate-100">
              ...
              <select
                ...
                onChange={(e) =>
                  setFilterCategory(e.target.value as ServiceCategory | "ALL")
                }
                className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-navy-900 outline-none focus:ring-2 focus:ring-gold-400"
              >
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-28 pl-6 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-navy-900 outline-none focus:ring-2 focus:ring-gold-400"
                />
              </div>
              {(filterCategory !== "ALL" || filterMinPay !== "") && (
                <button
                  onClick={() => {
            ...
            {!hasAuthorizedSkills && (
              <div className="p-6 bg-red-50 border border-red-100 rounded-2xl flex items-start gap-4 text-sm text-red-800 mb-6 shadow-sm">
                <Lock className="w-6 h-6 shrink-0 mt-0.5" />
                <div>
                  <p className="font-bold text-base">No Authorized Skills</p>
                  <p className="mt-1 opacity-90">
                    You have not been authorized for any job categories yet.
                  </p>
                </div>
              </div>
            )}

            {sortedAvailableGigs.eligible.length === 0 &&
            sortedAvailableGigs.ineligible.length === 0 ? (
              <div className="p-12 bg-white border-2 border-dashed border-slate-200 rounded-3xl text-center text-slate-400">
                No open gigs matching your filters.
              </div>
            ) : (
                ...
                {sortedAvailableGigs.ineligible.length > 0 && (
                  <div className="mt-8 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    <div className="opacity-75 hover:opacity-100 transition-opacity duration-300">
                      {sortedAvailableGigs.ineligible.map((gig) =>
                        renderGigCard(gig),
                      )}
                    </div>
                  </div>
          ...
          <div>
            ...
            <div className="bg-white p-6 rounded-3xl shadow-soft border border-slate-100">
                ...
                <div className="space-y-3">
                  {myActiveGigs.map((gig) => {
                    ...
                    return (
                      <div
                        key={gig.id}
                        className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-slate-200 transition-colors"
                      >
                        <div>
                          {gig.userId && (
                            <div className="flex items-center gap-1.5 mb-1.5">
                              {(() => {
                                const pro = users.find(
                                  (u) => u.id === gig.userId,
                                );
                                if (!pro) return null;
                                return (
                                  <>
                                    {pro.profileImage ? (
                                      <img
                                        ...
                                        alt={pro.name}
                                        className="w-5 h-5 rounded-full object-cover shadow-sm bg-slate-100"
                                      />
                                    ) : (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {isOwnPosting && (
                              <span className="text-[10px] font-black text-navy-500 bg-navy-100 px-2 py-0.5 rounded-full uppercase tracking-tighter">
                                My Posting
                              </span>
                            )}
                          </div>
                          <div className="text-xs text-slate-500 mt-0.5">
                            {format(gig.start, "MMM d, h:mm a")}
                          </div>
                        </div>
                        <div className="flex flex-col items-end gap-1">
                          ...
                          <div className="flex gap-2 mt-1">
                            {(isOwnPosting
                              ? users.find((u) => u.id === gig.userId)
                              : users.find((u) => u.id === gig.clientId)
                            )?.phone && (
                              <a
                                href={`tel:${(isOwnPosting ? users.find((u) => u.id === gig.userId) : users.find((u) => u.id === gig.clientId))?.phone}`}
                                onClick={(e) => handlePhoneClick(e, (isOwnPosting ? users.find((u) => u.id === gig.userId) : users.find((u) => u.id === gig.clientId))?.phone || '')}
                                className="text-[10px] font-bold text-slate-600 hover:text-slate-800 flex items-center bg-slate-100 px-2 py-1 rounded"
                              >
                                <Phone className="w-3 h-3 mr-1" /> Call
                              </a>
                            )}
                            {((isOwnPosting && gig.userId) ||
                              (!isOwnPosting && gig.clientId)) && (
                              <button
                                onClick={(e) => handleChatClick(e, gig)}
                                className="text-[10px] font-bold text-navy-600 hover:text-navy-800 flex items-center bg-navy-50 px-2 py-1 rounded"
                              >
              ...
              <button
                ...
                className="w-full mt-6 py-3 border-2 border-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-50 hover:border-slate-200 transition-all text-sm"
              >
                View Full Calendar
              </button>
            </div>
            <div className="mt-8 space-y-6">
              <div className="bg-gradient-to-br from-navy-900 to-navy-800 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <h3 className="font-bold text-gold-400 text-lg mb-1 uppercase tracking-wider text-xs">
                    Total Earnings
                  </h3>
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => navigate("/biz-center")}
                      className="w-full bg-white/10 hover:bg-white/20 py-3 rounded-xl text-sm font-bold transition-all backdrop-blur-sm border border-white/10"
                    >
                      View Details
                    </button>
                  </div>
              ...
              {isProviderReferralEnabled && (
                <div className="bg-white rounded-3xl p-6 shadow-soft border border-slate-100 flex items-center justify-between hover:shadow-lg transition-all">
                  <div>
                    <h3 className="font-bold text-navy-900 flex items-center mb-1">
                      <UserPlus className="w-5 h-5 mr-2 text-gold-500" /> Refer
                      a Pro
                    </h3>
                    <p className="text-xs text-slate-500 max-w-[200px] leading-relaxed">
                      Earn $50 for every skilled pro you bring to the crew.
                    </p>
                  </div>
                  <button
                    onClick={() => openReferralModal("PROVIDER")}
                    className="p-4 bg-navy-50 text-navy-600 rounded-2xl hover:bg-navy-100 hover:text-navy-700 transition-colors shadow-sm"
      ...
      {isRequestModalOpen && (
        <div className="fixed inset-0 bg-navy-950/80 flex items-center justify-center p-4 z-50 backdrop-blur-md">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-300 max-h-[90vh] overflow-y-auto">
            <div
              className={`p-6 text-white flex justify-between items-center ${isUrgent ? "bg-red-600" : "bg-navy-900"}`}
            >
              <h3 className="font-extrabold text-xl flex items-center tracking-tight">
                {isUrgent ? (
                  <Siren className="w-6 h-6 mr-3 text-white animate-pulse" />
                ) : (
                  <Zap className="w-6 h-6 mr-3 text-gold-400" />
                )}
                {editingGig
                  ? "Update Request"
                  : `Request a Pro: ${selectedCategory}`}
              </h3>
              <button
            ...
            <div className="p-8 space-y-6">
              <div
                className={`flex items-center justify-between p-4 rounded-xl border-2 transition-all cursor-pointer ${isUrgent ? "bg-red-50 border-red-500 shadow-inner" : "bg-slate-50 border-slate-200"}`}
                onClick={() => setIsUrgent(!isUrgent)}
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-full ${isUrgent ? "bg-red-200 text-red-700" : "bg-slate-200 text-slate-500"}`}
                  >
                    <AlertTriangle className="w-6 h-6" />
                  </div>
                  <div>
                    <p
                      className={`font-bold text-sm ${isUrgent ? "text-red-700" : "text-slate-600"}`}
                    >
                    ...
                    <div className="text-[10px] text-slate-500 mt-1 leading-relaxed max-w-xs">
                      {isUrgent ? (
                        <span className="font-bold text-red-600">
                          There is a flat $25 service fee added for Emergency
                          Requests to encourage our Pro's to quickly grab these
                          jobs. Even so, better offers=faster response.
                        </span>
                      ) : (
                ...
                <div
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${isUrgent ? "bg-red-600" : "bg-slate-300"}`}
                >
                  <span
              ...
              <div
                className={`flex flex-col p-4 rounded-xl border-2 transition-all ${isRecurringJob ? "bg-indigo-50 border-indigo-500 shadow-inner" : "bg-slate-50 border-slate-200"}`}
              >
                <div
                  className="flex items-center justify-between cursor-pointer"
                  onClick={() => setIsRecurringJob(!isRecurringJob)}
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-2 rounded-full ${isRecurringJob ? "bg-indigo-200 text-indigo-700" : "bg-slate-200 text-slate-500"}`}
                    >
                      <RotateCcw className="w-6 h-6" />
                    </div>
                    <div>
                      <p
                        className={`font-bold text-sm ${isRecurringJob ? "text-indigo-700" : "text-slate-600"}`}
                      >
                      ...
                      <div className="text-[10px] text-slate-500 mt-1 leading-relaxed max-w-xs">
                        {isRecurringJob ? (
                          <span className="font-bold text-indigo-600">
                            This will create a subscription. The pro can accept
                            this ongoing work.
                          </span>
                        ) : (
                          "If you need this done regularly (like mowing or cleaning), set it up once and we'll handle the rest."
                        )}
                      </div>
                    </div>
                  </div>
                  <div
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${isRecurringJob ? "bg-indigo-600" : "bg-slate-300"}`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${isRecurringJob ? "translate-x-6" : "translate-x-1"}`}
                    />
                  </div>
                </div>
                {isRecurringJob && (
                  <div className="mt-4 pt-4 border-t border-indigo-100 flex gap-2 overflow-x-auto pb-2">
                    {["TWICE_A_WEEK", "WEEKLY", "BIWEEKLY", "MONTHLY"].map(
                      (freq) => (
                        <button
                          key={freq}
                          type="button"
                          onClick={() => setRecurringFrequency(freq as any)}
                          className={`flex-1 min-w-[80px] py-2 text-xs font-bold rounded-lg transition-colors ${recurringFrequency === freq ? "bg-indigo-600 text-white" : "bg-white text-indigo-600 border border-indigo-200 hover:bg-indigo-50"}`}
                        >
                          {freq === "BIWEEKLY"
                            ? "BI-WEEKLY"
                            : freq === "TWICE_A_WEEK"
              ...
              {selectedCategory &&
                (() => {
                  const catDef = serviceCategories.find(
              ...
              {selectedCategory &&
              (CATEGORY_RISK_MAPPING[selectedCategory]?.risk ||
                RISK_LEVELS.LOW) === RISK_LEVELS.HIGH ? (
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-indigo-700 mt-1 leading-relaxed">
                        For <strong>{selectedCategory}</strong>, providers will
                        review your details and submit competitive quotes. You
                        can review their profiles and prices before hiring.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl">
                  ...
                  >
                    <Zap
                      className={`w-3 h-3 mr-1.5 ${selectionMethod === "QUICK_BID" ? "text-gold-500" : ""}`}
                    />
                    Quick Bid
                  </button>
                  <button
                  ...
                  >
                    <Scale
                      className={`w-3 h-3 mr-1.5 ${selectionMethod === "BIDDING" ? "text-indigo-500" : ""}`}
                    />
                    Request Quotes
                  </button>
                </div>
                ...
                {descriptionAiUsed && (
                  <div className="mt-2 p-3 bg-gold-100 border border-gold-200 rounded-xl flex items-start gap-3 animate-in slide-in-from-top-2 duration-300">
                    ...
                    <div className="text-xs text-gold-900 font-medium">
                      ...
                      Please review and update the text above. Ensure all
                      details are accurate before posting.
                    </div>
                  </div>
                )}
                ...
                {requestImages.length > 0 && (
                  <div className="mt-4 flex gap-2 flex-wrap">
                    {requestImages.map((img, idx) => (
                      <div
                        key={idx}
                        className="relative w-16 h-16 rounded-lg border border-slate-200 overflow-hidden group"
                      >
                        <img
                          src={img}
                          alt={`upload-${idx}`}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-gold-100 focus:border-gold-400 transition-all text-sm font-medium"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-navy-900 mb-2">
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full p-3 bg-white border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-navy-100 text-sm font-medium"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-1">
                    ...
                    <div className="flex gap-2 justify-end pt-2">
                      ...
                      <button
                        type="button"
                        onClick={handleSaveInlineSite}
                        disabled={!newLocationName || !newLocationAddress}
                        className="px-4 py-2 text-sm font-bold text-white bg-navy-900 hover:bg-navy-800 rounded-xl transition-colors disabled:opacity-50"
                      >
                        Save & Use
                      </button>
                    </div>
              ...
              <div>
                  ...
                  {priceSuggestion && (
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() =>
                          setEstimatedPrice(priceSuggestion.avg.toString())
                        }
                        className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100 hover:bg-emerald-100 transition-colors flex items-center gap-1"
                      >
                      ...
                      {priceSuggestion.examples.length > 0 && (
                        <button
                          type="button"
                          onClick={() =>
                            setShowPriceExamples(!showPriceExamples)
                          }
                          className="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded-lg border border-slate-200 hover:bg-slate-200 transition-colors flex items-center gap-1"
                        >
                          See examples{" "}
                          {showPriceExamples ? (
                ...
                <input
                  ...
                  placeholder={
                    priceSuggestion ? `e.g. ${priceSuggestion.avg}` : "50"
                  }
                  className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-gold-100 focus:border-gold-400 outline-none font-bold text-lg transition-all"
                />
                  ...
                  <div className="mt-4 bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 rounded-xl p-4 animate-in fade-in slide-in-from-top-2 shadow-sm relative overflow-hidden">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs font-bold text-navy-900 uppercase tracking-wide flex items-center gap-1">
                            Market Insight
                            <span className="bg-indigo-100 text-indigo-700 text-[8px] px-1.5 py-0.5 rounded-full font-black uppercase tracking-tighter">
                              AI
                            </span>
                          </p>
                          <p className="text-[10px] text-slate-400">
                            Harrisburg, PA Area • Apr 2026
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="p-2 bg-amber-50 border border-amber-100 rounded-lg flex items-start gap-2 mb-3">
                      ...
                      <p className="text-[10px] text-amber-800 leading-tight">
                        <strong>AI Disclaimer:</strong> This estimate is based
                        on typical rates for similar jobs. AI content can be
                        inaccurate; always review and adjust to your specific
                        needs.
                      </p>
                    </div>
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          setEstimatedPrice(
                            Math.round(
                              (marketEstimate.min + marketEstimate.max) / 2,
                            ).toString(),
                          )
                        }
                    ...
                    <div className="mt-4 bg-blue-50 border border-blue-100 rounded-xl p-4 animate-in slide-in-from-top-2">
                      ...
                      <div className="space-y-3">
                        {priceSuggestion.examples.map((ex: any) => (
                          <div
                            key={ex.id}
                            className="flex justify-between items-start text-xs bg-white p-3 rounded-lg border border-blue-100 shadow-sm"
                          >
                            <div className="flex-1 pr-4">
                              ...
                              <span className="text-slate-400 block">
                                {ex.completedAt
                                  ? formatDistanceToNow(
                                      new Date(ex.completedAt),
                                    ) + " ago"
                                  : "Recently"}
                              </span>
                            </div>
                            <span className="font-black text-emerald-600 bg-emerald-50 px-2 py-1 rounded border border-emerald-100 whitespace-nowrap">
                              ${ex.price}
                            </span>
                          </div>
                        ))}
              ...
              {crewListPros.length > 0 && !editingGig && (
                <div className="pt-4 border-t border-slate-100">
                  <label className="block text-sm font-bold text-navy-900 mb-2">
                    Target Your Crew (Optional)
                  </label>
                  <p className="text-xs text-slate-500 mb-3">
                    Send this opportunity directly to professionals on your Crew
                    List.
                  </p>
                  ...
                  <div className="space-y-2 mb-4 max-h-40 overflow-y-auto pr-2">
                    {crewListPros.map((pro) => (
                      ...
                      >
                        ...
                        <div
                          className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${targetCrewIds.includes(pro.id) ? "border-navy-500 bg-navy-500 text-white" : "border-slate-300"}`}
                        >
                          {targetCrewIds.includes(pro.id) && (
                  ...
                  {targetCrewIds.length > 0 && (
                    <label className="flex items-center p-3 bg-slate-50 rounded-xl cursor-pointer mb-4">
                      ...
                      <span className="text-sm font-medium text-navy-900">
                        Alert Crew First (2-hour delay before public posting)
                      </span>
                    </label>
                  )}
                </div>
              ...
              <button
                ...
                disabled={!requestDesc || !estimatedPrice}
                className={`w-full py-4 text-white font-extrabold text-lg rounded-2xl shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all mt-2 disabled:opacity-50 disabled:hover:translate-y-0 ${isUrgent ? "bg-red-600 hover:bg-red-700 shadow-red-200" : "bg-gold-400 text-navy-900 hover:bg-gold-500 shadow-gold-200"}`}
              >
                {editingGig
                  ? "UPDATE REQUEST"
                  : isUrgent
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-extrabold text-navy-900 flex items-center">
                <CreditCard className="w-6 h-6 mr-2 text-blue-600" /> Payment
                Required
              </h2>
              <button
            ...
            <p className="text-sm text-slate-500 mb-6 leading-relaxed">
              To prevent spam and ensure providers get paid, we require a valid
              payment method on file before you can post a request.
              <br />
              <br />
              <strong>No charge is made now.</strong> We only place a hold when
              a provider accepts your job.
            </p>
            <div className="space-y-4">
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 relative overflow-hidden group">
                <div className="flex items-center justify-between mb-4">
                  <span className="font-bold text-navy-900 tracking-widest">
                    4242 4242 4242 4242
                  </span>
                ...
                <div className="absolute inset-0 bg-white/60 flex items-center justify-center backdrop-blur-[1px] opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                  <span className="text-xs font-bold text-navy-900 bg-white px-2 py-1 rounded shadow-sm">
                    Test Card
                  </span>
                </div>
              </div>
              <div className="bg-amber-50 border border-amber-100 p-3 rounded-xl text-[10px] text-amber-800 font-medium">
                <p className="font-bold mb-1 flex items-center">
                  <Info className="w-3 h-3 mr-1" /> Test Card Info:
                </p>
              ...
              <button
                onClick={() => {
                  if (currentUser) {
                    ...
                    setIsPaymentModalOpen(false);
                    alert(
                      "Card added successfully! You can now post your request.",
                    );
                  }
                }}
                className="w-full py-4 bg-navy-900 text-white font-bold rounded-xl hover:bg-navy-800 shadow-lg transition-all hover:-translate-y-0.5 flex items-center justify-center"
              >
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-extrabold text-navy-900 flex items-center">
                <Briefcase className="w-6 h-6 mr-2 text-gold-500" /> Apply for
                Skill
              </h3>
              <button
            ...
            <div className="mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
              <p className="text-sm text-slate-600 font-medium">
                You are requesting authorization for:
              </p>
              <p className="text-lg font-black text-navy-900 mt-1">
                {skillApplyCategory}
              </p>
            </div>
      ...
      {counterGig && (
        <div className="fixed inset-0 bg-navy-950/80 flex items-center justify-center p-4 z-50 backdrop-blur-md animate-in fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-extrabold text-navy-900 flex items-center">
                <Scale className="w-6 h-6 mr-2 text-gold-500" /> Make Counter
                Offer
              </h3>
              <button
                onClick={() => setCounterGig(null)}
                className="p-2 hover:bg-slate-50 rounded-full transition-colors"
              >
                <X className="w-5 h-5 text-slate-400" />
              </button>
            ...
            <div className="mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
              ...
              <div className="relative">
                <p
                  className={`font-bold text-navy-900 text-lg ${expandedGigId === counterGig.id ? "" : "line-clamp-4"}`}
                >
                ...
                {counterGig.description.length > 150 && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setExpandedGigId(
                        expandedGigId === counterGig.id ? null : counterGig.id,
                      );
                    }}
                    className="text-xs font-bold text-indigo-600 hover:text-indigo-800 mt-1 flex items-center"
                  >
                    {expandedGigId === counterGig.id
                      ? "Show Less"
                      : "Read More"}{" "}
                    <ChevronRight
                      className={`w-3 h-3 ml-1 transition-transform ${expandedGigId === counterGig.id ? "-rotate-90" : "rotate-90"}`}
                    />
                  </button>
                )}
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                {(() => {
                  ...
                  const platformFeePercent = platformConfig?.[
                    counterGig.category
                  ]?.platformFeePercent
                    ? platformConfig[counterGig.category].platformFeePercent /
                      100
                    : 0.15;
                    ...
                    <div className="mt-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
                      ...
                      {isZelle && (
                        <p className="mt-2 text-xs text-indigo-600 font-medium bg-indigo-50 p-2 rounded flex gap-1.5 items-start">
                          ...
                          You are using Zelle; no further transaction or
                          subscription fees will be deducted from this payout.
                        </p>
                      )}
                    </div>
                  );
        ...
        <div className="fixed inset-0 bg-navy-950/80 flex items-center justify-center p-4 z-50 backdrop-blur-md animate-in fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100 transition-all">
              ...
              {isJobDetailsOpen ? (
                <p className="font-bold text-navy-900 text-sm mt-2 animate-in fade-in slide-in-from-top-1 leading-relaxed border-t border-slate-200 pt-2">
                  {quoteGig.description}
                </p>
              ) : (
                <p className="font-bold text-navy-900 text-sm mt-1 truncate opacity-70">
                  {quoteGig.description}
                </p>
              )}
            ...
            <div className="space-y-4">
              <div>
                ...
                {(() => {
                  ...
                  if (bidAmount <= 0) return null;
                  const platformFeePercent = platformConfig?.[quoteGig.category]
                    ?.platformFeePercent
                    ? platformConfig[quoteGig.category].platformFeePercent / 100
                    : 0.15;
                  const platformCut = bidAmount * platformFeePercent;
                  const takeHome = bidAmount - platformCut;
                  const isZelle = currentUser?.payoutMethod === "ZELLE";
                  ...
                  <div className="mt-2 p-3 bg-indigo-100 border border-indigo-200 rounded-lg flex items-start gap-2 animate-in slide-in-from-top-1 duration-300">
                    ...
                    <div className="text-[11px] text-indigo-900 font-medium italic">
                      ...
                      <span
                        ...
                        title="AI may produce inaccuracies. Always review before sending."
                      >
                        Review and personalize
                      </span>{" "}
                      your message before submitting your bid.
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="fixed inset-0 z-[60] overflow-y-auto bg-navy-950/80 backdrop-blur-md animate-in fade-in">
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95 relative">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-extrabold text-navy-900 flex items-center">
                  <Calendar className="w-6 h-6 mr-2 text-indigo-500" /> Confirm
                  Schedule
                </h3>
                <button
              ...
              <div className="mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-sm text-slate-600 mb-4">
                  Please confirm the dates for this job before proceeding to
                  payment.
                </p>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                ...
                <button
                  onClick={() => {
                    if (confirmQuote.isQuote) {
                      handleAcceptQuote(
                        confirmQuote.offer as Quote,
                        finalDate,
                        finalEndDate,
                      );
                    } else {
                      handleAcceptCounter(
                        confirmQuote.offer as CounterOffer,
                        finalDate,
                        finalEndDate,
                      );
                    }
        ...
        <PaymentModal
          ...
          onSuccess={(pi) => {
            if (currentUser) {
              ...
              alert(
                "Payment successful! Our team will review your background check shortly. Once activated, you'll be able to bid on jobs.",
              );
            }
          }}
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95 relative">
              <div className="flex justify-between items-center mb-6">
                ...
                <button
                  onClick={() => setVerificationGig(null)}
                  className="p-2 hover:bg-slate-50 rounded-full transition-colors"
                >
                  <X className="w-5 h-5 text-slate-400" />
                </button>
              ...
              <div className="mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
                ...
                <div className="max-h-32 overflow-y-auto pr-2 custom-scrollbar">
                  <p className="font-bold text-navy-900 text-lg mb-2">
                    {verificationGig.description}
                  </p>
                </div>
                  ...
                  {verificationGig.appliedPlatformFee !== undefined && (
                    <div className="flex justify-between items-center text-xs text-slate-400 italic">
                      ...
                      <span>
                        -$
                        {(
                          (verificationGig.appliedPlatformFee > 1
                            ? verificationGig.appliedPlatformFee / 100
                            : verificationGig.appliedPlatformFee) *
                          (verificationGig.price || 0)
                        ).toFixed(2)}
                      </span>
                    </div>
                  )}
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-navy-950/80 backdrop-blur-md animate-in fade-in">
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-6 animate-in zoom-in-95 relative">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-extrabold text-navy-900 flex items-center">
                  <Scale className="w-6 h-6 mr-2 text-indigo-600" /> Review
                  Offers
                </h3>
                <button
                  onClick={() => setReviewOffersGig(null)}
                  className="p-2 hover:bg-slate-50 rounded-full transition-colors"
                >
                  <X className="w-5 h-5 text-slate-400" />
                </button>
              ...
              <div className="mb-6">
                <div className="relative">
                  <h4
                    className={`font-bold text-navy-900 mb-1 ${expandedGigId === reviewOffersGig.id ? "" : "line-clamp-4"}`}
                  >
                  ...
                  {reviewOffersGig.description.length > 150 && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setExpandedGigId(
                          expandedGigId === reviewOffersGig.id
                            ? null
                            : reviewOffersGig.id,
                        );
                      }}
                    ...
                    >
                      {expandedGigId === reviewOffersGig.id
                        ...
                        : "Read More"}{" "}
                      <ChevronRight
                        className={`w-3 h-3 ml-1 transition-transform ${expandedGigId === reviewOffersGig.id ? "-rotate-90" : "rotate-90"}`}
                      />
                    </button>
                  )}
                </div>
                <p className="text-sm text-slate-500">
                  Original Budget: ${reviewOffersGig.price}
                </p>
              </div>
              ...
              <div className="space-y-4">
                {(() => {
                  const pendingCounters =
                    reviewOffersGig.counterOffers?.filter(
                      (o) => o.status === "PENDING",
                    ) || [];
                  const pendingQuotes =
                    reviewOffersGig.quotes?.filter(
                      (q) => q.status === "PENDING",
                    ) || [];
                  ...
                  return allOffers.map((offer) => {
                    const provider = users.find(
                      (u) => u.id === offer.providerId,
                    );
                    const isQuote = offer.type === "QUOTE";
                    const isProAlreadyAssignedInGroup = !!(
                      reviewOffersGig.groupId &&
                      shifts.some(
                        (s) =>
                          s.groupId === reviewOffersGig.groupId &&
                          s.id !== reviewOffersGig.id &&
                          s.userId === provider?.id &&
                          s.status !== ShiftStatus.CANCELLED,
                      )
                    );
                    const { rating: displayRating } = getProviderStats(
                      provider?.id,
                      shifts,
                      provider,
                    );
                    ...
                    return (
                      <div
                        key={offer.id}
                        className="bg-slate-50 p-5 rounded-2xl border border-slate-200 hover:border-indigo-200 transition-colors"
                      >
                        <div className="flex justify-between items-start mb-3">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-slate-500 flex items-center gap-2">
                                <span className="flex items-center">
                                  <Star className="w-3 h-3 text-gold-400 mr-1 fill-current" />{" "}
                                  {displayRating} Rating
                                </span>
                                {isProAlreadyAssignedInGroup && (
                                  <span className="text-xxs bg-red-50 text-red-600 font-extrabold border border-red-100 px-2 py-0.5 rounded-full uppercase font-sans">
                                    Already Assigned
                                  </span>
                                )}
                              </div>
                          ...
                          <div className="text-right">
                            ...
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                              {offer.amount > (reviewOffersGig.price || 0)
                                ? `+$${(offer.amount - (reviewOffersGig.price || 0)).toFixed(2)}`
                                : "Match"}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="bg-white p-3 rounded-xl border border-slate-100 text-sm text-slate-600 italic mb-4 relative">
                          ...
                          <div
                            className={`relative ${expandedOfferId === offer.id ? "" : "line-clamp-4"}`}
                          >
                            "{offer.message}"
                          </div>
                          {offer.message && offer.message.length > 150 && (
                            ...
                            >
                              {expandedOfferId === offer.id
                                ...
                                : "Read More"}{" "}
                              <ChevronRight
                                className={`w-3 h-3 ml-1 transition-transform ${expandedOfferId === offer.id ? "-rotate-90" : "rotate-90"}`}
                              />
                            </button>
                          )}
                        </div>
                        ...
                        <div className="flex gap-3">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              isQuote
                                ? handleDeclineQuote(offer as Quote)
                                : handleDeclineCounter(offer as CounterOffer);
                            }}
                          ...
                          <button
                            onClick={() => {
                              ...
                              const dateToUse = offer.suggestedStart
                                ? new Date(offer.suggestedStart)
                                : reviewOffersGig.start
                                  ? new Date(reviewOffersGig.start)
                                  : new Date();
                              setFinalDate(
                              ...
                              const endDateToUse = offer.suggestedEnd
                                ...
                                : offer.suggestedStart
                                  ? new Date(offer.suggestedStart)
                                  : reviewOffersGig.end
                                    ? new Date(reviewOffersGig.end)
                                    : new Date();
                              setFinalEndDate(
                                endDateToUse.toISOString().split("T")[0],
                              );
                            }}
                            disabled={isProAlreadyAssignedInGroup}
                            className={`flex-1 py-2.5 font-bold rounded-xl shadow-md transition-all text-sm flex items-center justify-center ${isProAlreadyAssignedInGroup ? "bg-slate-100 text-slate-400 cursor-not-allowed" : "bg-indigo-600 text-white hover:bg-indigo-700 hover:shadow-lg"}`}
                          >
                            {isProAlreadyAssignedInGroup
                              ? "Duplicate Assigned"
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() =>
                      copyToClipboard(
                        `https://app.crew.com/signup?ref=${currentUser?.id}&type=${referralType.toLowerCase()}`,
                      )
                    }
      ...
      {facebookShareData && (
        <FacebookShareModal
          isOpen={!!facebookShareData}
          onClose={() => setFacebookShareData(null)}
          gig={facebookShareData.gig}
          reviewFeedback={facebookShareData.feedback}
          providerName={facebookShareData.providerName}
        />
      )}

      {toast && (
        <div className="fixed bottom-6 right-6 z-50 max-w-sm w-full bg-white rounded-2xl shadow-2xl border border-slate-100 p-4 animate-in slide-in-from-bottom duration-300">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-extrabold text-navy-900 leading-tight">
                {toast.type === "success" ? "Success" : toast.type === "error" ? "Alert" : "Notice"}
              </p>
