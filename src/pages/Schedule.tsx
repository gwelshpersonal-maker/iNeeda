...
const REQUIRE_GPS_VERIFICATION = false;
...
export const Schedule = () => {
  ...
  const [confirmQuote, setConfirmQuote] = useState<{ quoteId: string } | null>(
    null,
  );
  ...
  // Geofence states
  const [providerDistance, setProviderDistance] = useState<number | null>(null);
  const [isGettingLocation, setIsGettingLocation] = useState(false);
  const [manualOverrideActive, setManualOverrideActive] = useState(false);
  const [overrideReason, setOverrideReason] = useState("");
  const [overrideImages, setOverrideImages] = useState<string[]>([]);
  ...
  // Geofence calculation effect
  useEffect(() => {
    if (!selectedShift || currentUser?.role !== Role.PROVIDER) {
    ...
    const isCheckInPending = 
      selectedShift.status === ShiftStatus.ACCEPTED || 
      selectedShift.status === ShiftStatus.EN_ROUTE;
    ...
    if (!site || site.latitude === undefined || site.longitude === undefined) {
      setProviderDistance(0);
      return;
    }
    ...
    getCurrentPosition()
      .then((pos) => {
        ...
        setProviderDistance(distance);
      })
      .catch((err) => {
        console.error("Failed to get location for geofence check:", err);
        setProviderDistance(null); // Location lookup failed
      })
      .finally(() => {
        setIsGettingLocation(false);
      });
  }, [selectedShift, sites, currentUser]);
  ...
  const handleSmsClick = (e: React.MouseEvent<HTMLAnchorElement>, phone: string) => {
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)
      || window.matchMedia('(pointer: coarse)').matches;
    ...
    e.stopPropagation();
    navigator.clipboard.writeText(phone).then(() => {
      setToast({ message: `Text number copied: ${phone}`, type: "success" });
    }).catch((err) => {
      console.error("Failed to copy text number:", err);
    });
  };
  ...
  const filteredShifts = useMemo(() => {
    return shifts.filter((s) => {
      ...
      if (isProvider)
        return s.userId === currentUser?.id || s.clientId === currentUser?.id;
      return false;
    });
  ...
  const shiftsOnDate = useMemo(() => {
    return filteredShifts.filter(
      (s) => s.start && isSameDay(new Date(s.start), selectedDate),
    );
  ...
  const handleVerifyCompletion = async () => {
    ...
    try {
      ...
      if (verificationRating === 5) {
        const providerName =
          users.find((u) => u.id === selectedShift.userId)?.name || "a Pro";
  ...
  const handleAcceptCounter = async (offerId: string) => {
    ...
    try {
      ...
      let insuranceFee = 0;
      if (shouldDeduct) {
        insuranceFee = 2.0; // Flat fee fallback
      }
  ...
  const handleAcceptQuote = async (quoteId: string, selectedDate?: string) => {
    ...
    try {
      ...
      const newEnd = selectedDate
        ? addHours(new Date(selectedDate), 2)
        : selectedShift.end;
  ...
  const handleImageUpload = (
    ...
    type: "PRE_WORK" | "COMPLETION" | "ARRIVAL" | "OVERRIDE",
  ) => {
    const files = e.target.files;
    if (files) {
      const currentCount =
        type === "PRE_WORK"
          ? preWorkImages.length
          : type === "ARRIVAL"
            ? arrivalImages.length
            : type === "OVERRIDE"
              ? overrideImages.length
        ...
        reader.onload = (event) => {
          ...
          img.onload = () => {
            ...
            if (type === "PRE_WORK") {
              ...
              setOverrideImages((prev) => [...prev, compressedBase64]);
            } else {
  ...
  const removeImage = (
    ...
    if (type === "PRE_WORK") {
      ...
      const newImages = overrideImages.filter((_, i) => i !== index);
      setOverrideImages(newImages);
    } else {
  ...
  const handleOverrideCheckIn = async () => {
    ...
    if (overrideImages.length === 0) {
      alert("Override Photo Required: You MUST upload a photo of the job site to request an override.", "error");
      return;
    }
    ...
    try {
      ...
      const updatedJob: Shift = {
        ...
        arrivalPhotos: overrideImages,
        geofenceOverridePending: true,
        geofenceOverridePhoto: overrideImages[0],
        geofenceOverrideReason: overrideReason || "Poor reception or GPS drift.",
      };

      await updateShift(updatedJob);
      setSelectedShift(updatedJob);
      alert("Check-in submitted via override. The shift has started and will be reviewed by the client.", "success");
      setManualOverrideActive(false);
      setOverrideImages([]);
      setOverrideReason("");
    } catch (err: any) {
      console.error("Override check-in failed:", err);
      let msg = "Failed to submit override check-in. Please try again.";
  ...
  const handleCheckIn = async () => {
    ...
    if (arrivalImages.length === 0) {
      alert(
        "Arrival Photo Required: You MUST upload at least one photo showing the site upon arrival before checking in.",
      );
      return;
    }
    ...
    try {
      ...
      if (REQUIRE_GPS_VERIFICATION) {
        alert(
          "GPS Verification Required: We cannot verify your arrival without a location fix. Please ensure location services are enabled and try again.",
        );
        setIsSubmitting(false);
        return;
      }
    ...
    try {
      ...
      setSelectedShift(updatedJob);
      alert(
        "Success! You have checked in at the job site. The job is now in progress.",
      );
    } catch (err: any) {
      ...
      try {
        ...
        if (parsed.error && parsed.error.includes("quota")) {
          errorMessage =
            "Storage quota exceeded. Please try again later or upload smaller photos.";
        }
      } catch (e) {
  ...
  const handleCompleteJob = async () => {
    ...
    if (selectedShift.hasHighValueItems && preWorkImages.length === 0) {
      alert(
        "STOP: This job involves high-value items. You MUST upload at least one pre-work photo showing condition before you can complete the job.",
      );
      return;
    }
    ...
    if (completionImages.length === 0) {
      alert(
        "Proof of Work Required: You MUST upload at least one photo showing the completed work to close this job.",
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        (selectedShift.hasHighValueItems || (selectedShift.price || 0) > 200)
      ) {
        alert(
          "GPS LOCK REQUIRED: We cannot verify service delivery without a location fix. Please ensure location services are enabled and try again.",
        );
        setIsSubmitting(false);
        return;
      }
    ...
    try {
      ...
      if (errorStr.includes("too large") || errorStr.includes("limit")) {
        msg =
          "The job data (especially photos) is too large for the current system limits. Please remove some photos or use smaller ones.";
      } else {
        try {
  ...
  const renderShiftCard = (shift: Shift) => {
    ...
    const pendingOffers =
      shift.counterOffers?.filter((o) => o.status === "PENDING") || [];
    const pendingQuotes =
      shift.quotes?.filter((q) => q.status === "PENDING") || [];
    const totalPending = pendingOffers.length + pendingQuotes.length;
    const hasPendingOffers =
      totalPending > 0 && shift.status === ShiftStatus.OPEN_REQUEST;

    const isBoosting = boostingGigId === shift.id;
    const isBoostSuccess = boostSuccessId === shift.id;
    const assignedProvider = shift.userId
    ...
    return (
      ...
      >
        ...
        <div className="flex items-start gap-4">
          <div
            className={`p-3 rounded-xl font-bold text-center min-w-[70px] ${isCompleted ? "bg-white text-emerald-600 shadow-sm" : "bg-slate-50 text-slate-500"}`}
          >
            <div className="text-xs uppercase">
              {format(shift.start, "MMM")}
            </div>
            <div className="text-xl text-navy-900">
              {format(shift.start, "d")}
            </div>
          </div>
          <div>
            {assignedProvider && (
              <div className="flex items-center gap-1.5 mb-1.5">
                {assignedProvider.profileImage ? (
                  <img
                    src={assignedProvider.profileImage}
                ...
                <span className="text-xs font-bold text-navy-600">
                  Assigned Local Pro: {assignedProvider.name}
                </span>
              </div>
            )}
            <div className="relative">
              <h3
                className={`font-bold text-navy-900 text-lg ${expandedShiftId === shift.id ? "" : "line-clamp-4"}`}
              >
                {shift.description}
              </h3>
              {shift.description.length > 150 && (
                ...
                >
                  {expandedShiftId === shift.id ? "Show Less" : "Read More"}{" "}
                  <ChevronRight
                    className={`w-3 h-3 ml-1 transition-transform ${expandedShiftId === shift.id ? "-rotate-90" : "rotate-90"}`}
                  />
                </button>
              )}
            </div>
            <div className="flex items-center gap-4 text-sm text-slate-500 mt-1">
              ...
              <span className="flex items-center">
                <MapPin className="w-4 h-4 mr-1" /> {site?.name}
              </span>
            </div>
          </div>
        </div>
        <div className="flex flex-col items-end gap-2 mt-2 md:mt-0">
          ...
          {hasPendingOffers && (
            <span className="text-[10px] font-bold bg-indigo-600 text-white px-2 py-0.5 rounded-full animate-pulse flex items-center">
              <Scale className="w-3 h-3 mr-1" /> {totalPending} NEW BIDS
            </span>
          )}
          {shift.price && (
            <span
              className={`font-black text-navy-900 ${shift.isBoosted ? "text-purple-600" : ""}`}
            >
              ${shift.price}
            </span>
          )}
          ...
          {/* Rebook Button for Clients on Past Jobs */}
          {isClient && isVerified && shift.userId && (
            <div className="mt-2 flex flex-col gap-2">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  const rebookCategory = shift.category;
                  navigate(
                    `/?rebook=${shift.userId}&category=${rebookCategory}`,
                  );
                }}
                className="text-xs font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg flex items-center transition-colors"
              >
                <RotateCcw className="w-3 h-3 mr-1" /> Rebook Same Pro
              </button>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  navigate(`/?templateId=${shift.id}`);
                }}
                className="text-xs font-bold text-navy-600 hover:text-navy-800 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-lg flex items-center transition-colors border border-slate-200"
              >
                <Plus className="w-3 h-3 mr-1" /> Re-post as New Job
              </button>
            </div>
          )}

          {/* Follow up offer for Providers */}
          {isProvider && isVerified && shift.clientId && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                alert(
                  "Follow-up offer feature coming soon: Send a direct proposal to this past client.",
                );
              }}
              className="mt-2 text-xs font-bold text-emerald-600 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 px-3 py-1.5 rounded-lg flex items-center transition-colors"
            >
              <Sparkles className="w-3 h-3 mr-1" /> Send Follow-up Offer
            </button>
          )}
            ...
            <div className="flex gap-2 mt-2">
              {(() => {
                const contactUser = users.find(
                  (u) => u.id === (isClient ? shift.userId : shift.clientId),
                );
                ...
                return (
                  <>
                    <a
                      ...
                      onClick={(e) => handlePhoneClick(e, contactUser.phone || '')}
                    ...
                    <a
                      ...
                      onClick={(e) => handleSmsClick(e, contactUser.phone || '')}
                      className="p-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-full transition-colors group"
                      title="Text Message"
                    >
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
        ...
        <div className="space-y-4">
          ...
          {readyForWorkJobs.map((job) => {
            ...
            return (
              ...
              >
                <div
                  className={`absolute top-0 left-0 w-1 h-full transition-colors ${isEnRoute ? "bg-blue-500" : isInProgress ? "bg-emerald-500" : "bg-gold-400 group-hover:bg-gold-500"}`}
                ></div>
        ...
        <>
          ...
          <div className="grid grid-cols-7 gap-2 md:gap-4">
            {weekDays.map((day) => {
              ...
              const hasShift = filteredShifts.some((s) =>
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-lg font-black mt-1 ${isSelected ? "text-gold-400" : "text-slate-700"}`}
                  >
                    {format(day, "d")}
                  </span>
                  {hasShift && (
        ...
        <div className="fixed inset-0 bg-navy-950/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-6 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              {(selectedShift.isFrozen || selectedShift.escrowStatus === 'FROZEN') && (
                <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-xl">
                  <h4 className="font-bold text-red-800 flex items-center">
                    <AlertTriangle className="w-5 h-5 mr-2" /> Payout Frozen
                  </h4>
                  <p className="text-sm text-red-700 mt-1">
                    This shift's payout has been frozen by an administrator. Verification, check-ins, and payouts are locked.
                  </p>
                </div>
              )}
              ...
              {isProvider && (
                <>
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div>
                        ...
                        >
                          {sites.find((s) => s.id === selectedShift.siteId)
                    ...
                    <div className="flex items-start gap-3">
                      <div className="h-5 w-5 rounded-full bg-navy-100 flex items-center justify-center text-navy-600 text-xs font-bold shrink-0 mt-0.5">
                        {users
                          .find((u) => u.id === selectedShift.clientId)
                          ?.name.charAt(0)}
                      </div>
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-600 mb-2">
                          {users.find((u) => u.id === selectedShift.clientId)
                            ?.name || "Unknown Client"}
                        </p>
                        ...
                        <div className="flex gap-2">
                          {users.find((u) => u.id === selectedShift.clientId)
                            ?.phone && (
                            <a
                              href={`tel:${users.find((u) => u.id === selectedShift.clientId)?.phone}`}
                              onClick={(e) => handlePhoneClick(e, users.find((u) => u.id === selectedShift.clientId)?.phone || '')}
                              className="text-xs font-bold bg-white border border-slate-200 px-3 py-1.5 rounded-lg flex items-center hover:bg-slate-50 transition-colors"
                            >
                          ...
                          <button
                            onClick={() =>
                              navigate(`/chat/${selectedShift.id}`)
                            }
                  ...
                  {selectedShift.preWorkPhotos &&
                    selectedShift.preWorkPhotos.length > 0 && (
                      <div className="bg-purple-50 p-4 rounded-xl border border-purple-200">
                        <label className="block text-sm font-bold text-purple-900 mb-2 flex items-center">
                          <AlertCircle className="w-4 h-4 mr-2" /> Pre-Work
                          Condition Photos
                        </label>
                        <div className="flex gap-3 overflow-x-auto pb-2">
                          {selectedShift.preWorkPhotos.map((img, i) => (
                            <div
                              key={i}
                              className="w-20 h-20 rounded-xl overflow-hidden border border-purple-200 shrink-0 shadow-sm"
                            >
                              <img
                                src={img}
                                alt={`Pre-work ${i}`}
                  ...
                  {selectedShift.photos && selectedShift.photos.length > 0 && (
                    <div>
                      <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center">
                        <ImageIcon className="w-4 h-4 mr-1" /> Initial Site
                        Photos
                      </h3>
                      <div className="flex gap-3 overflow-x-auto pb-2">
                        {selectedShift.photos.map((photo, i) => (
                          <div
                            key={i}
                            className="w-24 h-24 rounded-xl overflow-hidden border border-slate-200 shrink-0 shadow-sm"
                          >
                            <img
                              src={photo}
                              alt={`Job site ${i}`}
                  ...
                  {selectedShift.completionPhotos &&
                    selectedShift.completionPhotos.length > 0 && (
                      <div>
                        <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center">
                  ...
                  <div className="bg-gold-50 p-4 rounded-xl border border-gold-200">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="font-black text-xl text-navy-900">
                        ${selectedShift.price}
                      </span>
                    </div>
                    {selectedShift.providerFeedback &&
                      selectedShift.providerFeedback.includes(
                        "[PRICE ADJUSTMENT]",
                      ) && (
                  ...
                  {(selectedShift.status === ShiftStatus.ACCEPTED ||
                    selectedShift.status === ShiftStatus.EN_ROUTE ||
                    selectedShift.status === ShiftStatus.IN_PROGRESS) &&
                    !selectedShift.isFrozen &&
                    selectedShift.escrowStatus !== 'FROZEN' && (
                      ...
                      selectedShift.status === ShiftStatus.EN_ROUTE ? (
                        <div className="mb-6">
                          <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 mb-6">
                            ...
                            {isGettingLocation ? (
                              <div className="flex items-center gap-2 py-2 text-xs font-bold text-slate-500">
                                <Loader2 className="w-4 h-4 animate-spin text-blue-500" /> Calculating your distance from job site...
                              </div>
                            ) : providerDistance !== null ? (
                              <div className={`p-3 rounded-lg text-xs font-bold mb-4 ${providerDistance <= 200 ? 'bg-emerald-50 text-emerald-800 border border-emerald-100' : 'bg-red-50 text-red-800 border border-red-100'}`}>
                                {providerDistance <= 200 ? (
                                  <span>You are within geofence range ({Math.round(providerDistance)} meters away).</span>
                                ) : (
                                  <span>Geofence Alert: You are {Math.round(providerDistance)} meters away (Must be within 200 meters).</span>
                                )}
                              </div>
                            ) : (
                              <div className="bg-amber-50 text-amber-800 border border-amber-100 p-3 rounded-lg text-xs font-bold mb-4">
                                Location coordinates could not be verified. You can use Manual Override below.
                              </div>
                            )}
                            ...
                            {!manualOverrideActive ? (
                              <>
                                ...
                                <div className="flex flex-wrap gap-3 mb-2">
                                  {arrivalImages.map((img, idx) => (
                                    <div
                                      key={idx}
                                      className="w-20 h-20 relative rounded-xl overflow-hidden border border-blue-200 shadow-sm group"
                                    >
                                      <img
                                        src={img}
                                        alt="Arrival"
                              ...
                              <div className="bg-amber-50/50 p-4 rounded-xl border border-amber-200 space-y-4 animate-in slide-in-from-top-2 duration-200">
                                <div>
                                  <label className="block text-xs font-bold text-amber-900 mb-2">
                                    Override Verification Photo (Required)
                                  </label>
                                  <div className="flex flex-wrap gap-3 mb-2">
                                    {overrideImages.map((img, idx) => (
                                      <div
                                        key={idx}
                                        className="w-20 h-20 relative rounded-xl overflow-hidden border border-amber-200 shadow-sm group"
                                      >
                                        <img
                                          src={img}
                                          alt="Override Proof"
                                        ...
                                        />
                                        <button
                                          onClick={() => removeImage(idx, "OVERRIDE")}
                                          className="absolute top-1 right-1 p-1 bg-white/80 rounded-full text-red-600 opacity-0 group-hover:opacity-100 transition-opacity"
                                        >
                                          <X className="w-3 h-3" />
                                        </button>
                                    ...
                                    <label className="w-20 h-20 flex flex-col items-center justify-center border-2 border-dashed border-amber-300 rounded-xl cursor-pointer hover:border-amber-500 hover:bg-amber-100 transition-colors text-amber-500 hover:text-amber-700">
                                      <Camera className="w-6 h-6 mb-1" />
                                      <span className="text-[10px] font-bold">Add</span>
                                      <input
                                        type="file"
                                        ...
                                        multiple
                                        className="hidden"
                                        onChange={(e) => handleImageUpload(e, "OVERRIDE")}
                                      />
                                    </label>
                                  </div>
                                </div>
                                ...
                                <div>
                                  ...
                                  <textarea
                                    ...
                                    placeholder="Explain why you are requesting manual check-in (e.g. poor cell signal, GPS drift)"
                                    className="w-full p-2.5 bg-white border border-amber-200 rounded-xl text-xs outline-none focus:border-amber-500"
                                  />
                                </div>
                              </div>
                            )}
                          ...
                          {!manualOverrideActive ? (
                            <div className="space-y-3">
                              <button
                                ...
                                disabled={isSubmitting || isGettingLocation || providerDistance === null || providerDistance > 200 || arrivalImages.length === 0}
                                className="w-full py-4 text-white text-lg font-bold rounded-xl shadow-lg transition-all flex items-center justify-center bg-blue-600 hover:bg-blue-700 shadow-blue-200 disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                {isSubmitting ? (
                                  "Checking In..."
                                ) : (
                              ...
                              {(providerDistance === null || providerDistance > 200) && (
                                <div className="text-center">
                                  <button
                                    onClick={() => setManualOverrideActive(true)}
                                    className="text-xs font-bold text-amber-700 hover:text-amber-800 underline decoration-dotted underline-offset-4 cursor-pointer"
                                  >
                                    GPS Drift or Signal Issues? Use Manual Override
                                  </button>
                                </div>
                              )}
                            </div>
                            ...
                            <div className="space-y-3">
                              <button
                                ...
                                disabled={isSubmitting || overrideImages.length === 0}
                                className="w-full py-4 text-white text-lg font-bold rounded-xl shadow-lg transition-all flex items-center justify-center bg-amber-600 hover:bg-amber-700 shadow-amber-200 disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                {isSubmitting ? "Submitting Override..." : "Submit Manual Override Check-in"}
                              </button>
                              <div className="text-center">
                                <button
                                  onClick={() => setManualOverrideActive(false)}
                                  className="text-xs font-bold text-slate-400 hover:text-slate-600 underline decoration-dotted underline-offset-4 cursor-pointer"
                                >
                                  Cancel Override & Check Geofence Again
                                </button>
                              </div>
                            </div>
                          )}
                        ...
                        <>
                          ...
                          {selectedShift.hasHighValueItems && (
                            <div className="mb-6 bg-purple-50 p-4 rounded-xl border border-purple-200">
                              <label className="block text-sm font-bold text-purple-900 mb-2 flex items-center">
                                ...
                                Required: Proof of Condition
                              </label>
                              <p className="text-xs text-purple-700 mb-3">
                                You must upload photos of high-value items{" "}
                                <strong>before</strong> handling them to protect
                              ...
                              <div className="flex flex-wrap gap-3">
                                {preWorkImages.map((img, idx) => (
                                  <div
                                    key={idx}
                                    className="w-20 h-20 relative rounded-xl overflow-hidden border border-purple-200 shadow-sm group"
                                  >
                                    <img
                                      src={img}
                                      alt="Pre-work condition"
                                      className="w-full h-full object-cover"
                                    />
                                    <button
                                      onClick={() =>
                                        removeImage(idx, "PRE_WORK")
                                      }
                                      className="absolute top-1 right-1 p-1 bg-white/80 rounded-full text-red-600 opacity-0 group-hover:opacity-100 transition-opacity"
                                    >
                                      <X className="w-3 h-3" />
                              ...
                              <div className="mt-3 pt-3 border-t border-gold-200 animate-in slide-in-from-top-2">
                                <div className="flex items-start gap-2 mb-2">
                                  ...
                                  <span className="text-xs font-bold text-amber-700">
                                    Price changed. Please explain why:
                                  </span>
                                </div>
                                <textarea
                                  ...
                                  onChange={(e) =>
                                    setPriceChangeReason(e.target.value)
                                  }
                                />
                              </div>
                            )}
                          ...
                          <div className="mb-6">
                            <label className="block text-sm font-bold text-navy-900 mb-2">
                              Proof of Work (Completion Photos){" "}
                            ...
                            <div className="flex flex-wrap gap-3">
                              {completionImages.map((img, idx) => (
                                <div
                                  key={idx}
                                  className="w-20 h-20 relative rounded-xl overflow-hidden border border-slate-200 shadow-sm group"
                                >
                                  <img
                                    src={img}
                              ...
                              <label className="w-20 h-20 flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-xl cursor-pointer hover:border-gold-400 hover:bg-gold-50 transition-colors text-slate-400 hover:text-gold-600">
                                ...
                                <input
                                  ...
                                  className="hidden"
                                  onChange={(e) =>
                                    handleImageUpload(e, "COMPLETION")
                                  }
                                />
                              </label>
                            </div>
                            {completionImages.length === 0 && (
                              <p className="text-xs text-amber-600 mt-2 font-medium flex items-center">
                                ...
                                Mandatory: Upload at least one photo.
                              </p>
                            )}
                          </div>
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="bg-amber-50 p-3 rounded-xl border border-amber-100 flex items-start text-xs text-amber-800 shadow-sm">
                              ...
                              <p>
                                <strong>GPS Verification Required:</strong>{" "}
                                Please mark this job complete while physically
                                at the job site. We capture your location at the
                                moment of completion to verify service delivery
                            ...
                            >
                              {isSubmitting ? (
                                ...
                                preWorkImages.length === 0 ? (
                                  <>
                                    <Lock className="w-5 h-5 mr-2" /> Upload
                                    Proof of Condition
                                  </>
                                ) : (
                                  <>
                                    <Camera className="w-5 h-5 mr-2" /> Upload
                                    Proof of Work
                                  </>
                                )
                              ) : (
                                <>
                                  <CheckCircle2 className="w-6 h-6 mr-2" /> Mark
                                  Job Complete
                                </>
                              )}
                            </button>
              ...
              {!isProvider && (
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-200">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 font-bold">
                          {users
                            .find((u) => u.id === selectedShift.userId)
                            ?.name.charAt(0)}
                        </div>
                        <div className="flex-1">
                          <div className="font-bold text-navy-900 text-sm">
                            {
                              users.find((u) => u.id === selectedShift.userId)
                                ?.name
                            }
                          </div>
                          <div className="flex gap-2 mt-2">
                            {users.find((u) => u.id === selectedShift.userId)
                              ?.phone && (
                              <a
                                href={`tel:${users.find((u) => u.id === selectedShift.userId)?.phone}`}
                                onClick={(e) => handlePhoneClick(e, users.find((u) => u.id === selectedShift.userId)?.phone || '')}
                                className="text-xs font-bold bg-white border border-slate-200 px-3 py-1.5 rounded-lg flex items-center hover:bg-slate-50 transition-colors"
                              >
                                <Phone className="w-3 h-3 mr-1" /> Call
                              </a>
                      ...
                      {/* COI Download Section */}
                      {isClient &&
                        users.find((u) => u.id === selectedShift.userId)
                          ?.insuranceType && (
                          <div className="mt-3 bg-emerald-50 rounded-xl p-3 border border-emerald-100 flex justify-between items-center">
                            <div className="flex items-center text-xs text-emerald-800">
                              <ShieldCheck className="w-4 h-4 mr-2" />
                              <div>
                                <strong className="block">Fully Covered</strong>
                                <span>
                                  {users.find(
                                    (u) => u.id === selectedShift.userId,
                                  )?.insuranceType === "DAILY_SHIELD"
                                    ? "Platform Daily Shield"
                                    : "Verified Commercial COI"}
                                </span>
                              </div>
                            </div>
                            <button
                              onClick={() => {
                                const coiUrl = users.find(
                                  (u) => u.id === selectedShift.userId,
                                )?.coiUrl;
                                if (coiUrl) {
                                  const a = document.createElement("a");
                                  a.href = coiUrl;
                                  a.download = `COI_${users.find((u) => u.id === selectedShift.userId)?.name.replace(/\s+/g, "_")}.png`;
                                  a.click();
                                } else {
                                  alert(
                                    "Certificate document is securely held by our risk team. A verified coverage certificate will be generated and downloaded in a future update.",
                                  );
                                }
                              }}
                              className="px-3 py-1.5 bg-white border border-emerald-200 text-emerald-700 text-xs font-bold rounded-lg hover:bg-emerald-50 transition-colors shadow-sm"
                            >
                              Download COI file
                            </button>
                          </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-200">
                      ...
                      {selectedShift.providerFeedback &&
                        selectedShift.providerFeedback.includes(
                          ...
                          <div className="text-xs text-amber-700 bg-amber-100/50 p-2 rounded border border-amber-200 mt-2">
                            <strong>Price Adjustment Note:</strong>{" "}
                            {selectedShift.providerFeedback}
                          </div>
                        )}
                    </div>
                  ...
                  {/* Client No-Show Management Flow */}
                  {isClient && (selectedShift.status === ShiftStatus.ACCEPTED || selectedShift.status === ShiftStatus.EN_ROUTE) && (() => {
                      const startVal = selectedShift.start ? new Date(selectedShift.start) : null;
                      if (!startVal) return null;
                      const now = new Date();
                      const twoHoursPast = new Date(startVal.getTime() + 2 * 60 * 60 * 1000);
                      if (now < twoHoursPast) return null;

                      return (
                          <div className="mt-4 pt-4 border-t border-slate-200 space-y-4">
                              <div className="bg-red-50 border-2 border-red-200 rounded-xl p-4 shadow-sm space-y-3">
                                  <h4 className="font-bold text-red-800 flex items-center text-xs uppercase tracking-wider">
                                      <AlertTriangle className="w-4 h-4 mr-1.5 text-red-600 animate-pulse" /> Provider No-Show Alert
                                  </h4>
                                  <p className="text-xs text-red-700">
                                      The provider has not checked in for this shift (Scheduled start was {format(startVal, "h:mm a")}). You can choose to extend the grace period or cancel/refund.
                                  </p>
                                  <div className="flex gap-2">
                                      <button
                                          onClick={async () => {
                                              const nowPlusHour = new Date(Date.now() + 60 * 60 * 1000);
                                              const nowStr = format(nowPlusHour, "yyyy-MM-dd'T'HH:mm");
                                              const newTime = prompt("Enter new start date and time:", nowStr);
                                              if (!newTime) return;
                                              try {
                                                  const token = await auth.currentUser?.getIdToken();
                                                  const response = await fetch(`/api/shifts/${selectedShift.id}/extend-grace-period`, {
                                                      method: 'POST',
                                                      headers: {
                                                          'Content-Type': 'application/json',
                                                          'Authorization': `Bearer ${token}`
                                                      },
                                                      body: JSON.stringify({ newStartTime: new Date(newTime).toISOString() })
                                                  });
                                                  if (response.ok) {
                                                      ...
                                                      alert(`Failed to extend grace period: ${errData.error}`);
                                                  }
                                              } catch (err: any) {
                                                  alert(`Error: ${err.message}`);
                                              }
                                          }}
                                          className="flex-1 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg text-xs transition-colors cursor-pointer text-center"
                                      >
                                          Extend Grace Period
                                      </button>
                                      <button
                                          onClick={async () => {
                                              if (!confirm("Are you sure you want to cancel this shift and request a full refund?")) return;
                                              try {
                                                  const token = await auth.currentUser?.getIdToken();
                                                  const response = await fetch(`/api/shifts/${selectedShift.id}/cancel-no-show`, {
                                                      method: 'POST',
                                                      headers: {
                                                          'Content-Type': 'application/json',
                                                          'Authorization': `Bearer ${token}`
                                                  ...
                                                  if (response.ok) {
                                                      alert("Shift cancelled and refund initiated.");
                                                      window.location.reload();
                                                  } else {
                                                      const errData = await response.json();
                                                      alert(`Failed to cancel shift: ${errData.error}`);
                                                  }
                                              } catch (err: any) {
                                                  alert(`Error: ${err.message}`);
                                              }
                                          }}
                                          className="flex-1 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg text-xs transition-colors cursor-pointer text-center"
                                      >
                                          Cancel & Refund
                                      </button>
                                  </div>
              ...
              {isClient &&
                selectedShift.status === ShiftStatus.OPEN_REQUEST && (
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        {boostingGigId === selectedShift.id ? (
                    ...
                    {/* Counter Offers */}
                    {selectedShift.counterOffers &&
                      selectedShift.counterOffers.length > 0 && (
                        <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4">
                          <h3 className="font-bold text-indigo-900 mb-4 flex items-center">
                            <Scale className="w-5 h-5 mr-2" /> Pending Counter
                            Offers
                          </h3>
                          <div className="space-y-3">
                            {selectedShift.counterOffers
                              ...
                              .map((offer) => {
                                ...
                                const isProAlreadyAssignedInGroup = !!(
                                  selectedShift.groupId &&
                                  shifts.some(
                                    (s) =>
                                      s.groupId === selectedShift.groupId &&
                                ...
                                return (
                                  ...
                                  >
                                        ...
                                        <div>
                                          ...
                                          <div className="text-xs text-slate-500 flex items-center gap-2">
                                            ...
                                            {isProAlreadyAssignedInGroup && (
                                              <span className="text-xxs bg-red-50 text-red-600 font-extrabold border border-red-100 px-2 py-0.5 rounded-full uppercase">
                                                Already Assigned
                                              </span>
                                            )}
                                          </div>
                                    ...
                                    <div className="flex gap-2">
                                      ...
                                      <button
                                        onClick={() =>
                                          handleAcceptCounter(offer.id)
                                        }
                                        disabled={isProAlreadyAssignedInGroup}
                                        className={`flex-1 py-2 font-bold rounded-lg transition-colors text-sm ${isProAlreadyAssignedInGroup ? "bg-slate-100 text-slate-400 cursor-not-allowed" : "bg-indigo-600 text-white hover:bg-indigo-700"}`}
                                      >
                                        {isProAlreadyAssignedInGroup
                                          ? "Duplicate Assigned"
                                          : "Accept Offer"}
                            ...
                            {selectedShift.counterOffers.filter(
                              (o) => o.status === "PENDING",
                            ).length === 0 && (
                    ...
                    {/* Quotes (Bids) */}
                    {selectedShift.quotes &&
                      selectedShift.quotes.length > 0 && (
                        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                          <h3 className="font-bold text-emerald-900 mb-4 flex items-center">
                            <TrendingUp className="w-5 h-5 mr-2" /> Competitive
                            Bids
                          </h3>
                          <div className="space-y-3">
                            {selectedShift.quotes
                              ...
                              .map((quote) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div className="flex gap-2">
                                      <button className="flex-1 py-2 bg-white border border-slate-200 text-slate-500 font-bold rounded-lg hover:bg-slate-50 transition-colors text-sm">
                                        Decline
                                      </button>
                                      <button
                                        onClick={() => {
                                        ...
                                        disabled={isProAlreadyAssignedInGroup}
                                        className={`flex-1 py-2 font-bold rounded-lg transition-colors text-sm ${isProAlreadyAssignedInGroup ? "bg-slate-100 text-slate-400 cursor-not-allowed" : "bg-emerald-600 text-white hover:bg-emerald-700"}`}
                                      >
                                        {isProAlreadyAssignedInGroup
                                          ? "Duplicate Assigned"
                                          : "Accept Bid"}
              ...
              {/* Client Geofence Override Review Flow */}
              {isClient && selectedShift.geofenceOverridePending &&
                !selectedShift.isFrozen &&
                selectedShift.escrowStatus !== 'FROZEN' && (
                <div className="bg-white border-2 border-amber-400 rounded-xl p-6 shadow-md relative overflow-hidden mb-6">
                  <div className="absolute top-0 left-0 w-full h-1 bg-amber-400 animate-pulse"></div>
                  <h3 className="font-bold text-lg text-amber-900 mb-2 flex items-center">
                    <AlertCircle className="w-5 h-5 mr-2 text-amber-500 animate-pulse" />{" "}
                    GPS Geofence Override Request
                  </h3>
                  <p className="text-slate-600 text-sm mb-4">
                    The provider checked in using a manual override due to GPS drift or poor reception. Please review their arrival photo to confirm they are on-site.
                  </p>

                  {selectedShift.geofenceOverridePhoto && (
                    <div className="w-full h-48 rounded-lg overflow-hidden border border-amber-200 bg-white relative mb-4">
                      <img 
                        src={selectedShift.geofenceOverridePhoto} 
                        alt="GPS Override Proof" 
                        className="w-full h-full object-cover"
                      />
                    </div>
                  )}

                  {selectedShift.geofenceOverrideReason && (
                    <div className="bg-amber-50 p-3 rounded-lg text-xs text-amber-800 italic mb-4 border border-amber-100">
                      <strong>Reason given:</strong> "{selectedShift.geofenceOverrideReason}"
                    </div>
                  )}
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={async () => {
                        await updateShift({
                          ...selectedShift,
                          geofenceOverridePending: false,
                          geofenceOverrideApproved: true
                        });
                        alert("Geofence override approved successfully.", "success");
                        setSelectedShift({
                          ...selectedShift,
                          geofenceOverridePending: false,
                          geofenceOverrideApproved: true
                      ...
                      className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-sm transition-colors shadow-sm flex items-center justify-center cursor-pointer"
                    >
                      Approve Check-in
                    </button>
                    <button
                      onClick={async () => {
                        await updateShift({
                          ...
                          status: ShiftStatus.ACCEPTED,
                          geofenceOverridePending: false,
                          geofenceOverrideApproved: false,
                          isDisputed: true,
                          disputeReason: "Geofence check-in rejected by client."
                        });
                        alert("Check-in rejected and shift flagged as disputed.", "error");
                        setSelectedShift({
                          ...selectedShift,
                          status: ShiftStatus.ACCEPTED,
                          geofenceOverridePending: false,
                      ...
                      className="flex-1 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl text-sm transition-colors shadow-sm flex items-center justify-center cursor-pointer"
                    >
                      Reject Check-in
                    </button>
                  </div>
                ...
                selectedShift.escrowStatus !== 'FROZEN' && (
                  <div className="bg-white border-2 border-emerald-400 rounded-xl p-6 shadow-md relative overflow-hidden">
                    ...
                    <p className="text-slate-500 text-sm mb-4">
                      The provider has marked this job as complete. Please
                      review the proof of work and release funds.
                    </p>
                        ...
                        <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 mb-4 animate-in fade-in">
                          ...
                          <div className="flex gap-2 mb-3">
                            {[1, 2, 3, 4, 5].map((star) => (
                              ...
                              >
                                <Star
                                  className={`w-6 h-6 ${star <= verificationRating ? "fill-gold-400 text-gold-400" : "text-slate-300"}`}
                                />
                              </button>
                            ))}
                          </div>
                          <textarea
                            ...
                            onChange={(e) =>
                              setVerificationFeedback(e.target.value)
                            }
                          />
                        </div>
                        ...
                        <div className="flex gap-3">
                          ...
                          >
                            {isVerifying ? (
                              ...
                              <>
                                <CheckCircle2 className="w-5 h-5 mr-2" /> Verify
                                & Release Funds
                              </>
                            )}
                          </button>
                      ...
                      <div className="space-y-4 animate-in fade-in">
                        <div>
                          <label className="block text-sm font-bold text-red-700 mb-2 flex items-center">
                            <AlertTriangle className="w-4 h-4 mr-2" /> Report an
                            Issue
                          </label>
                          <textarea
                ...
                <div className="bg-slate-100 border border-slate-200 p-4 rounded-xl flex flex-col items-center justify-center text-center">
                  ...
                  <div className="flex mt-3 text-gold-400">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <Star
                        key={i}
                        className={`w-4 h-4 ${i <= (selectedShift.clientRating || 0) ? "fill-current" : "text-slate-300"}`}
                      />
                    ))}
                  </div>
                </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95">
            ...
            <div className="mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
              <p className="text-sm text-slate-600 mb-4">
                Please confirm the date for this job before proceeding to
                payment.
              </p>
              <label className="block text-sm font-bold text-navy-900 mb-2">
                Final Date
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() =>
                  handleAcceptQuote(confirmQuote.quoteId, finalDate)
                }
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 max-w-sm w-full bg-white rounded-2xl shadow-xl border border-slate-100 p-4 animate-in slide-in-from-bottom duration-300">
          <div className="flex items-start gap-3">
